...
if not st.session_state.state['running']:
    try:
        with st.spinner("系統喚醒中，自動登入 API..."):
...
if st.session_state.state['running']:
    # API 存活檢查
    try: st.session_state.api.list_accounts()
    except: st.session_state.api.login(API_KEY, SECRET_KEY)
    ...
    for s in all_snaps:
        ...
        if not ((min_vol_pct >= adj_mom_thr) or (vol_diff >= 50)): continue
        ...
        current_detecting.append({"代碼": code, "名稱": st.session_state.name_map.get(code), "次數": hits})
