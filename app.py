...
if st.session_state.state['running']:
    ...
    # [C] 篩選與即時紀錄更新
    current_detecting = [] # 本次掃描到的符合初步條件清單
    cat_hits = {}
    ...
    # [D] 顯示看板
    st.subheader("🔍 即時偵測中紀錄 (依觸發次數排序)")
    if current_detecting:
        # 依照次數遞減排序，讓強勢股頂在最上面
        df_detect = pd.DataFrame(current_detecting).sort_values("次數", ascending=False)
        st.dataframe(df_detect, use_container_width=True)
    
    st.divider()
    st.subheader("📊 今日正式通報紀錄 (Discord)")
    if st.session_state.state['history']:
