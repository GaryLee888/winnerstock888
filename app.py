...
def load_local_history():
    ...
    if os.path.exists(REPORT_FILE):
        ...
        # 如果檔案是昨天以前的，或是現在已經過開盤前夕(08:30)，則刪除舊檔重來
        if mtime.date() < now.date() or (now.hour == 8 and now.minute >= 30 and mtime.hour < 8):
...
st.set_page_config(page_title="當沖雷達-24H穩定版", layout="wide")
...
if st.session_state.state['running']:
    ...
    for s in all_snaps:
        ...
        y_vol = st.session_state.y_vol_map.get(code, 0) # 預設為 0
        
        # 🛡️ 修復 ZeroDivisionError: 如果昨日量為 0，改為 1 避免除以零
        ...
        if not ((min_vol_pct >= adj_mom_thr) or (vol_diff >= 50)) or ratio < (vol_base * vol_weight): continue
